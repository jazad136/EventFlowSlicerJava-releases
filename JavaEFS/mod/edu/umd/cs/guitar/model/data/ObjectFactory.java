...
//        return new CTW();
    ...
    public Slice createSlice(java.util.Collection<EventType> events)
    {
    	return createSlice(events.toArray(new EventType[0]));
    }
    public Slice createSlice(EventType... events)
    {
    	Slice s = new Slice();
    	if(events.length > 0) {
    		s.getEvents().addAll(Arrays.asList(events));
    		s.setMain(events[events.length-1]);
    	}
    	return s;
    }
}
