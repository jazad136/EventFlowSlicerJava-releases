...
public class TST2GraphvizEFS
{
	...
	public static int lBuf, edgeBuf, endBuf, nBuf;
	/**
	 * @param args
	 */
...
//			eResult.append(labels.get(i));
	...
	public static StringBuffer toGraphviz(TestCase[] tsts)
	{
		...
		// setup:
		char[] eLbl = new char[1];

		eLbl[0] = 'a';
		ArrayList<StepType> currentSteps;
		for(int i = 0; i  < tsts.length; i++) {
			currentSteps = new ArrayList<StepType>(tsts[i].getStep());
			for(int j = 1; j < tsts[i].getStep().size(); j++) {

				String nextId = currentSteps.get(j).getEventId();

//				currentSteps = new ArrayList<StepType>(tsts[i].getStep());
//				StepType step = currentSteps.get(j);
//				// add new node
...
//				// add new edge
//				if(j <= currentSteps.size()-2) {
//					int n1, n2;
//					n1 = j;
//					n2 = j+1;
//					setNewEdge(eventNames.get(index))
//				}
			}
		}
	...
	public static String getNextLabel(char[] eLbl)
	{
		String nextLbl = "";
	...
	public static char[] setNextLabel(char[] eLbl)
	{
		if(eLbl[eLbl.length-1] == 'z') {
			eLbl = ArrayTools.extendRangeChar(eLbl.length+1, eLbl);
		}
		return eLbl;
	}
	public static void setNewEdge(ArrayList<StringBuffer>[] currents, String source, String target)
	{
		source = eventIdCorrection(source);
		target = eventIdCorrection(target);
		String edgeName = source + "->" + target;
		StringBuffer edgeResult = new StringBuffer();
		edgeResult.append(edgeName);
		currents[edgeBuf].add(edgeResult);

		StringBuffer lResult = new StringBuffer();
		lResult.append("[");
		lResult.append(" label=\"");
		lResult.append(" " + source);
		currents[lBuf].add(lResult);
		...
		customResult.append("\"");
		customResult.append(" ];");
		currents[endBuf].add(customResult);
	}
